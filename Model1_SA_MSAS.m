function [rho50, sigma_z] = Model1_SA_MSAS(X1,X2,X3,X4)

% Created by Mao-Xin Wang (dr.maoxin.wang@gmail.com or maoxinwang@ust.hk)
% January 2025
%
% Model 1: Correlations among SAs (SA-SA pairs)
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% INPUT
%
%   X1   =  scalar or matrix of T1
%   X2   =  scalar or matrix of T2
%   X3   =  scalar or matrix of ksi (damping ratio, in percentage)
%   X4   =  scalar or matrix of I_HV (set to 1, 2, 3, or 4 for 
%       MS_H(T1)-AS_H(T2), MS_H(T1)-AS_V(T2), MS_V(T1)-AS_H(T2),
%                            or MS_V(T1)-AS_V(T2), respectively)
...
load coeff_Model1
X_min = [log(0.01) 	log(0.01) 	0.5 	1];
X_max = [log(5) 	log(5) 	30 	4];
[n_row,n_col] = size(X1);
n_data = n_row*n_col;
...
Y1 = tanh(tanh(tanh(X_norm*W_1+b_1)*W_2+b_2)*W_3+b_3)*W_out+b_out;
...
Y2 = tanh(tanh(tanh(X_norm*W_1+b_1)*W_2+b_2)*W_3+b_3)*W_out+b_out;
